...
void main() {
  ...
  float distToLine = abs(fract(scaledY - 0.5) - 0.5);
  float lines = smoothstep(0.12, 0.0, distToLine);
  float pattern = vMask * lines;

  float cycleT = fullT * uColorCycleSpeed;
  float rChannel = (pattern + lines * ridge) * (cos(blended.y + cycleT * 0.234) * 0.5 + 1.0);
  float gChannel = (pattern + lines * ridge) * (sin(blended.x + cycleT * 1.745) * 0.5 + 1.0);
  float bChannel = (pattern + lines * ridge) * (cos(blended.x + cycleT * 0.534) * 0.5 + 1.0);

  // Faint background, bright silver glow near the mouse interaction
  float currentBrightness = uBrightness * (0.05 + 0.95 * proximity);
  vec3 col = (rChannel * uColor1 + gChannel * uColor2 + bChannel * uColor3) * currentBrightness;
