...
export class IdeaController {
  ...
  async updateIdea(
    ...
    const updated = await this.ideaService.updateIdea(
      session.user.id,
      id,
      idea,
    );
  ...
  async getIdeas(
    @Req() req: Request,
    @Query() query: GetIdeasQueryDto,
  ): Promise<PaginatedResponseDto<IdeaResponseDto>> {
    const session = await auth.api.getSession({
      headers: req.headers,
    });
    if (!session) {
    ...
    return {
      ...result,
      data: result.data.map((i) => IdeaMapper.toResponseDto(i)),
    };
  }
